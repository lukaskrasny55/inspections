    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-lg p-4 max-w-3xl w-full space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Nakresliť technický výkres</h3>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 ml-2">Hrúbka:</span>
          {[2, 4, 8].map((w) => (
            <button
              key={w}
              onClick={() => setLineWidth(w)}
              className={`px-2 py-1 rounded text-xs ${lineWidth === w ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}
            >
              {w}
            </button>
          ))}
        ...
        <canvas
          ...
          className="w-full h-auto border border-slate-200 dark:border-slate-700 rounded-md touch-none cursor-crosshair bg-white dark:bg-slate-800"
