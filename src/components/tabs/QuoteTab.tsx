...
export default function QuoteTab({ inspection, onChange }: Props) {
  ...
  async function handleGenerateFromChecklist() {
    ...
    try {
      ...
      // auto-generated + checklist-generated lines first — anything the owner
      // has since edited by hand is marked source:'manual' by the update
      // endpoint, so it survives.
      const regenerable = active.lineItems.filter((li) => li.source === 'auto_calculated' || li.source === 'from_checklist')
      for (const li of regenerable) {
        await deleteQuoteLineItem(li.id)
      }
      const survivingItems = active.lineItems.filter((li) => li.source === 'manual')
      ...
      const drafts: {
        ...
        wastePercent: number
        source?: 'auto_calculated' | 'from_checklist'
      ...
      // Položky, ktoré si zákazník vyklikal (zaškrtol) v checkliste z katalógu —
      // toto je jediné miesto, kde sa zaškrtnutie v checkliste reálne premietne
      // do cenovej ponuky. Množstvo berieme z vyplnenej hodnoty v checkliste
      // (ak nie je vyplnená, predvolene 1 ks/j.), cenu z katalógovej ceny.
      inspection.technicalSolutionItems
        .filter((item) => item.isChecked)
        .forEach((item) => {
          const catalogItem = item.catalogItem
          if (!catalogItem) return
          const qty = item.valueNumber ? Number(item.valueNumber) : 1
          drafts.push({
            description: item.notes ? `${catalogItem.name} – ${item.notes}` : catalogItem.name,
            plannedQty: qty,
            unit: catalogItem.unit,
            section: 'main',
            unitPriceSnapshot: Number(catalogItem.defaultUnitPrice),
            wastePercent: 0,
            source: 'from_checklist',
            technicalSolutionItemId: item.id,
          })
        })
      ...
      for (const draft of drafts) {
        const created = await createQuoteLineItem({
          ...
          source: draft.source ?? 'auto_calculated',
          technicalSolutionItemId: draft.technicalSolutionItemId,
        })
        items = [...items, created]
      }
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between -mt-2">
            <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
              <a href={`/api/generate-quote-excel?id=${active.id}`} className="hover:text-slate-700 dark:hover:text-white underline decoration-dotted">
                Stiahnuť Excel (ponuka)
              </a>
