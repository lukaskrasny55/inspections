    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-50 mt-1">Materiálové skladby</h1>
        </div>
        <button
        ...
        {showNewForm && (
          <form onSubmit={handleCreate} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Názov skladby (napr. Alternatíva A)</label>
              <input
                autoFocus
                value={newName}
    ...
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 space-y-3">
      <div className="flex items-center justify-between">
        <input
          ...
          className="text-sm font-semibold text-slate-700 dark:text-slate-300 px-2 py-1 border border-transparent hover:border-slate-200 focus:border-brand-400 rounded focus:outline-none"
        />
        <div className="flex items-center gap-3">
          {status === 'saving' && <span className="text-xs text-slate-400">Ukladám…</span>}
          {status === 'saved' && <span className="text-xs text-green-600 dark:text-green-400">Uložené</span>}
      ...
      <div>
        <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Vrstvy skladby (jedna na riadok)</label>
        <textarea
          value={layers}
      ...
      <div>
        <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Postup prác</label>
        <textarea
          value={workSteps}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Vyzdvihnutý produkt (technická charakteristika)</label>
          <select
